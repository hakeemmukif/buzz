...
/// committed this head's row is gone and the flush skips it. And it holds
/// `FLUSH_PUBLISHER_LOCK` across its entire invocation, so no *second* flush
/// can publish the tombstone in the await gap between this flush's re-read and
/// its POST. Serialized flush ⟹ the only interleavings are head-before-tombstone
/// (head lands first, then dominated by the later tombstone) or
/// purged-row-skip (delete committed first, so the re-read skips the head) —
/// a purged head can never publish after its tombstone.
